...
import { CheckCircle2, Copy, Loader2, X, RefreshCw, Upload, ImageIcon, Clock } from 'lucide-react'
...
interface Props {
  amount: number
  orderId: string
  onSubmitted: () => void   // called after screenshot submitted (goes to pending screen)
  onCancel: () => void
}

type Step = 'loading' | 'qr' | 'uploading' | 'pending' | 'error'

export default function UPIPayment({ amount, orderId, onSubmitted, onCancel }: Props) {
  ...
  async function submitPayment() {
    ...
    try {
      // 1. Upload screenshot to your storage (Cloudinary example)
      const formData = new FormData()
      formData.append('file', screenshot)
      formData.append('orderId', orderId)

      const uploadRes = await fetch('/api/payments/upi/upload-screenshot', {
        method: 'POST',
        body: formData,
      })
      if (!uploadRes.ok) throw new Error((await uploadRes.json()).error ?? 'Upload failed')
      const { screenshotUrl } = await uploadRes.json()

      // 2. Submit confirmation with screenshot URL + optional UTR
      ...
      setStep('qr')
      setError(e instanceof Error ? e.message : 'Something went wrong. Email tesschstore@gmail.com with your screenshot.')
    }
  }
  ...
  if (step === 'pending') return (
    <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
      ...
      <div>
        <p className="font-display text-4xl uppercase tracking-tighter text-[#17191d] leading-none mb-3">
          Payment<br />Under<br /><span className="text-amber-500">Verification</span>
        </p>
        <p className="font-mono text-[10px] uppercase tracking-[2px] text-[#17191d]/50 max-w-[260px] mx-auto leading-relaxed mt-4">
          Our team is reviewing your payment screenshot. Your order will be confirmed within 30 minutes.
        </p>
      </div>

      <div className="w-full bg-amber-50 border-2 border-amber-200 px-5 py-4 text-left space-y-2">
        <p className="font-mono text-[9px] uppercase tracking-[3px] text-amber-700 font-bold">What happens next?</p>
        {[
          'We verify your payment screenshot',
          'Order confirmed & you get an email',
          'Your build ships in 1–3 business days',
        ].map((s, i) => (
          <div key={i} className="flex items-start gap-2">
            <span className="font-display text-amber-500 text-sm leading-none mt-0.5 shrink-0">{i + 1}</span>
            <p className="font-mono text-[9px] uppercase tracking-[1px] text-[#17191d]/60 leading-relaxed">{s}</p>
          </div>
        ))}
      </div>

      <p className="font-mono text-[9px] uppercase tracking-[2px] text-[#17191d]/30 leading-relaxed">
        You will receive an email at your registered address.<br />
        Questions? tesschstore@gmail.com
      </p>

      <button onClick={onSubmitted}
        className="font-mono text-[10px] font-bold uppercase tracking-[3px] border-2 border-[#17191d] px-8 py-4 hover:bg-[#17191d] hover:text-white transition-colors">
        Go to Homepage →
      </button>
    </motion.div>
  )
  ...
  if (step === 'uploading') return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
      ...
      <p className="font-mono text-[10px] uppercase tracking-[3px] text-[#17191d]/60">Submitting payment proof...</p>
    </motion.div>
  )
      ...
      <motion.div key="qr" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col gap-5">
        ...
        <div className="bg-[#17191d]/5 border-l-4 border-[#d4604d] pl-4 py-3 space-y-2">
          {[
            ...
            `Upload the screenshot below and click "Submit Proof"`,
          ].map((s, i) => (
            <div key={i} className="flex items-start gap-2">
        ...
        <div>
          ...
          {screenshotPreview ? (
            <div className="relative border-2 border-emerald-400 bg-emerald-50 p-3 flex items-start gap-3">
              <img src={screenshotPreview} alt="Payment proof" className="w-20 h-20 object-cover border border-[#17191d]/10" />
              <div className="flex-1 min-w-0">
                <p className="font-mono text-[10px] font-bold text-emerald-700 uppercase">Screenshot attached</p>
                <p className="font-mono text-[8px] text-[#17191d]/40 uppercase mt-1 truncate">{screenshot?.name}</p>
              </div>
              <button
                onClick={() => { setScreenshot(null); setScreenshotPreview(null); if (fileInputRef.current) fileInputRef.current.value = '' }}
                className="text-[#17191d]/30 hover:text-red-500 transition-colors shrink-0">
                <X size={16} />
              </button>
            </div>
          ) : (
            <button
              ...
              className="w-full border-2 border-dashed border-[#17191d]/30 bg-white hover:border-[#d4604d] hover:bg-[#d4604d]/5 transition-all py-8 flex flex-col items-center gap-3 group">
              <div className="w-10 h-10 border-2 border-[#17191d]/20 group-hover:border-[#d4604d] flex items-center justify-center transition-colors">
                <ImageIcon size={18} className="text-[#17191d]/30 group-hover:text-[#d4604d] transition-colors" />
              </div>
              <div className="text-center">
                <p className="font-mono text-[10px] font-bold uppercase tracking-[2px] text-[#17191d]/60 group-hover:text-[#d4604d] transition-colors flex items-center gap-2">
                  <Upload size={11} /> Upload Screenshot
                </p>
                <p className="font-mono text-[8px] uppercase tracking-[1px] text-[#17191d]/30 mt-1">JPG, PNG · Max 10MB</p>
              </div>
            </button>
          )}
          <input
        ...
        <div>
          <label className="font-mono text-[9px] uppercase tracking-[3px] text-[#17191d]/50 block mb-2">
            UTR / Transaction Reference <span className="opacity-40">(optional but speeds up verification)</span>
          </label>
          <input
            value={txnRef}
        ...
        {error && <p className="font-mono text-[9px] text-red-500 uppercase tracking-[1px]">⚠ {error}</p>}
        ...
        <div className="flex gap-3">
          ...
          <button onClick={submitPayment} disabled={!screenshot}
            className="flex-1 bg-[#d4604d] text-white font-mono text-[10px] font-bold uppercase tracking-[3px] py-4 hover:bg-[#17191d] transition-colors disabled:opacity-40 disabled:cursor-not-allowed">
            Submit Payment Proof →
          </button>
        </div>

        <p className="font-mono text-[8px] uppercase tracking-[1px] opacity-25 text-center leading-relaxed">
          Your order will be confirmed after our team verifies the screenshot. Usually under 30 minutes.
        </p>
      </motion.div>
    </AnimatePresence>
  )
