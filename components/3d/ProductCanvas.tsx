...
export default function ProductCanvas({
  ...
  useEffect(() => {
    ...
    // Fill Light (Fire Color) - CRASH FIXED: Object.assign REMOVED
    const fillLight = new THREE.PointLight(0xff3d00, 2, 12);
    ...
    const body = new THREE.Mesh(
      new THREE.CylinderGeometry(0, 1.1, 1.24, 9, 1, false, Math.PI * 0.13, Math.PI * 1.12),
      uMat
    );
    body.position.set(-0.26, 0.28, 0);
    body.rotation.z = 0.28;
    body.scale.set(1.75, 0.86, 0.72);
    group.add(body);
    
    const toe = new THREE.Mesh(new THREE.SphereGeometry(0.58, 12, 9), uMat);
    toe.position.set(1.04, -0.1, 0);
    toe.scale.set(1, 0.64, 0.64);
    group.add(toe);
    
    for (let i = 0; i < 4; i++) {
      const lace = new THREE.Mesh(
        new THREE.BoxGeometry(0.02, 0.04, 0.76),
      ...
      lace.position.set(-0.46 + i * 0.26, 0.32, 0);
      group.add(lace);
    }
    scene.add(group);
    ...
    const onMove = (e: PointerEvent) => {
      if (!isDrag) return;
      velY = (e.clientX - lx) * 0.0056;
      velX = (e.clientY - ly) * 0.0056;
