...
function markCodeUsed(code: string) {
  ...
  localStorage.setItem(
    USED_KEY,
    JSON.stringify([...new Set([...used, code.toUpperCase()])]),
  );
}
...
function validateCoupon(
  ...
  | { discountAmt: number; label: string; isOneTime: boolean }
  | { error: string } {
  ...
  if (named) {
    const raw =
      named.type === "flat"
        ? named.discount
        : Math.round(subtotal * named.discount);
...
export default function CheckoutPage() {
  ...
  async function placeOrder() {
    ...
    try {
      ...
      if (!res.ok)
        throw new Error((await res.json()).error ?? "Order creation failed");
        ...
        <div className="max-w-lg mx-auto">
          <div className="mb-8 border-b-2 border-[#17191d]/10 pb-6">
            ...
            <h1 className="font-display text-[clamp(36px,6vw,72px)] leading-none uppercase tracking-tighter">
              PAY VIA
              <br />
              <span className="text-[#d4604d]">UPI.</span>
            </h1>
          </div>
          ...
          <UPIPayment
            ...
            onSubmitted={() => {
              // This is called AFTER the user hits "Submit Proof"
              clearCart();
              router.push("/order-success-pending"); // Or just '/'
            }}
            onCancel={() => {
              setStep("confirm");
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-10 border-b-2 border-[#17191d] pb-6">
          ...
          <h1 className="font-display text-[clamp(36px,6vw,80px)] leading-none uppercase tracking-tighter">
            FINALISE
            <br />
            <span className="text-[#d4604d]">YOUR ORDER.</span>
          </h1>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-12">
              ...
              {step === "payment" && (
                ...
                >
                  ...
                  <div className="flex items-center gap-4 p-5 border-[3px] border-[#17191d] bg-[#17191d] text-white">
                    ...
                    <div>
                      <p className="font-mono text-[11px] font-bold uppercase tracking-[2px]">
                        UPI Payment
                      </p>
                      <p className="font-mono text-[9px] mt-0.5 opacity-60">
                        GPay · PhonePe · Paytm · BHIM — QR code at next step
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-emerald-50 border border-emerald-200 px-4 py-3">
                    <p className="font-mono text-[9px] text-emerald-700 uppercase tracking-[1px] font-bold">
                      🎉 Up to ₹{getMRPCap(subtotal).toLocaleString("en-IN")}{" "}
                      discount available on your order
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    {appliedCoupon ? (
                      <div className="flex items-center justify-between bg-emerald-50 border-2 border-emerald-400 px-4 py-3">
                        <div>
                          <p className="font-mono text-[10px] font-bold text-emerald-700 uppercase">
                            {appliedCoupon.code}
                          </p>
                          <p className="font-mono text-[9px] text-emerald-600">
                            {appliedCoupon.label} — saving ₹
                            {appliedCoupon.discountAmt.toLocaleString("en-IN")}
                          </p>
                        </div>
                        <button
                          onClick={() => setAppliedCoupon(null)}
                          className="font-mono text-[9px] text-[#17191d]/50 hover:text-red-500 transition-colors"
                        >
                          Remove ✕
                        </button>
                      </div>
                    ...
                    {couponError && (
                      <p className="font-mono text-[9px] text-red-500 mt-2 uppercase">
                        {couponError}
                      </p>
                    )}
                  </div>
                ...
                >
                  ...
                  <div className="flex flex-col gap-3">
                    {items.map((item) => {
                      ...
                      return (
                        <div
                          key={item.id}
                          className="flex items-center gap-4 bg-white border-2 border-[#17191d] p-4"
                        >
                          <img
                          ...
                          <div className="flex-1">
                            ...
                            <p className="font-mono text-[9px] uppercase tracking-[2px] opacity-40">
                              {item.type === "build"
                                ? `Full Build + ${item.sole?.name ?? "Sole"}`
                                : "Upper Only"}{" "}
                  ...
                  {selectedAddress && (
                    <div className="bg-white border-2 border-[#17191d] p-5 space-y-1">
                      <p className="font-mono text-[9px] uppercase tracking-[3px] opacity-40 mb-2">
                        Delivering To
                      </p>
                      ...
                      <p className="font-mono text-[10px] opacity-50 mt-2 uppercase">
                        Payment: UPI
                      </p>
                    </div>
                  )}
                  ...
                  {appliedCoupon && (
                    <div className="bg-emerald-50 border-2 border-emerald-300 px-5 py-3">
                      <p className="font-mono text-[9px] text-emerald-700 uppercase font-bold">
                        Coupon {appliedCoupon.code} — saving ₹
                        {appliedCoupon.discountAmt.toLocaleString("en-IN")}
                      </p>
                    </div>
                  )}
          ...
          <div className="lg:sticky lg:top-28 h-fit">
            <div className="bg-[#17191d] text-[#e5f1ee] p-8 flex flex-col gap-5">
              <p className="font-mono text-[9px] uppercase tracking-[4px] text-[#d4604d] font-bold">
                Price Breakdown
              </p>
              ...
              <div className="border-t border-[#e5f1ee]/10 pt-5 flex justify-between items-end">
                <p className="font-mono text-[9px] uppercase opacity-60">
                  You Pay
                </p>
              ...
              <div className="border-t border-[#e5f1ee]/10 pt-4">
                ...
                <p className="font-mono text-[8px] opacity-30 uppercase tracking-[1px] mt-1">
                  QR code shown at next step
                </p>
              </div>
            </div>
          </div>
