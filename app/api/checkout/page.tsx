...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    if (!isAuthenticated && typeof window !== 'undefined') router.push('/api/auth/login')
      ...
      const rzp = new window.Razorpay({
        ...
        handler: async (response: {
          ...
          if (verifyRes.ok) {
            ...
            setError('Payment verification failed. Contact support with your order ID: ' + internalOrder.id)
          }
        },
        modal: {
          ondismiss: () => {
  ...
  if (items.length === 0) return (
    <div className="min-h-screen bg-[#e5f1ee] flex items-center justify-center flex-col gap-6 text-[#17191d]">
      ...
      <button onClick={() => router.push('/products')} className="bg-[#17191d] text-white font-mono text-[10px] font-bold uppercase tracking-[3px] px-8 py-4 hover:bg-[#d4604d] transition-colors">Browse Products →</button>
    </div>
  )
              ...
              <AnimatePresence mode="wait">
                ...
                {step === 'delivery' && (
                  <motion.div key="delivery" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-6">
                    ...
                    <button onClick={() => { if (!selectedAddressId) { setError('Select or add an address'); return } setError(''); setStep('payment') }}
                  ...
                  <motion.div key="confirm" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex flex-col gap-6">
                    ...
                    {selectedAddress && (
                      <div className="bg-white border-2 border-[#17191d] p-5 space-y-1">
                        ...
                        <p className="font-mono text-[10px] opacity-60 mt-2 uppercase">Payment: {payMethod === 'cod' ? 'Cash on Delivery' : payMethod}</p>
                      </div>
                    )}
                ...
                <div className="flex flex-col gap-3 text-[10px] font-mono uppercase">
                  ...
                  {appliedCoupon && (
                    <div className="flex justify-between text-[#d4604d]">
                      <span>{appliedCoupon.code}</span><span>−₹{discountAmt.toLocaleString('en-IN')}</span>
                    </div>
                  )}
                  <div className="flex justify-between">
                  ...
                  {payMethod === 'cod' && <div className="flex justify-between opacity-60"><span>COD Handling</span><span>₹49</span></div>}
                </div>
