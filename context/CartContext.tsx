...
export interface CartItem {
  id: string // This MUST eventually be the Prisma ID
  upper: UpperItem
  sole?: SoleItem
  size: string
  quantity: number
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  // ── FIX 1: SYNC ADDITION & GET REAL DB ID ────────────────────────────────
  const addItem = useCallback(async (
    ...
    const tempId = `${upper.id}_${size}`
    
    // Optimistic Update
    setItems(prev => {
      const existing = prev.find(i => i.id === tempId || i.id.length > 20) // simplistic check for DB vs Temp ID
      if (existing && (existing.id === tempId || existing.upper.id === upper.id && existing.size === size)) {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // SWAP TEMP ID FOR REAL PRISMA ID
  ...
  // ── FIX 2: DELETE USING REAL ID ──────────────────────────────────────────
  const removeItem = useCallback((id: string) => {
    setItems(prev => prev.filter(i => i.id !== id))
    if (!id.includes('_')) { // Only hit API if it's a real synced ID
      fetch(`/api/cart/${id}`, { method: 'DELETE' }).catch(() => {})
    }
  }, [])

  // ── FIX 3: UPDATE USING REAL ID ──────────────────────────────────────────
